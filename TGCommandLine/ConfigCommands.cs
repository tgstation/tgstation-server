...
namespace TGCommandLine
{
	...
	class ConfigDownloadCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var bytes = Service.GetComponent<ITGConfig>(Program.Instance).ReadRaw(parameters[0], parameters.Count > 2 && parameters[2].ToLower() == "--repo", out string error);
			if(bytes == null)
			{
	...
	class ConfigUploadCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			try
			{
				var res = Service.GetComponent<ITGConfig>(Program.Instance).WriteRaw(parameters[0], File.ReadAllText(parameters[1]));
				if (res != null)
				{

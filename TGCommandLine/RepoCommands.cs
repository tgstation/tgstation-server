...
namespace TGCommandLine
{
	...
	class RepoSetupCommand : Command
	{
		...
		{
			var res = Service.GetComponent<ITGRepository>().Setup(parameters[0], parameters.Count > 1 ? parameters[1] : "master");
			if (res != null)
			{
	...
	class RepoStatusCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var Repo = Service.GetComponent<ITGRepository>();
	...
	class RepoResetCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var result = Service.GetComponent<ITGRepository>().Reset(parameters.Count > 0 && parameters[0].ToLower() == "--origin");
	...
	class RepoUpdateCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			...
			var res = Service.GetComponent<ITGRepository>().Update(hard);
	...
	class RepoChangelogCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var result = Service.GetComponent<ITGRepository>().GenerateChangelog(out string error);
	...
	class RepoSetEmailCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			Service.GetComponent<ITGRepository>().SetCommitterEmail(parameters[0]);
			return ExitCode.Normal;
		}
	...
	class RepoSetNameCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			Service.GetComponent<ITGRepository>().SetCommitterName(parameters[0]);
			return ExitCode.Normal;
		}
		protected override string GetArgumentString()
		{
	...
	class RepoPythonPathCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			Service.GetComponent<ITGRepository>().SetPythonPath(parameters[0]);
			return ExitCode.Normal;
		}
		protected override string GetArgumentString()
		{
	...
	class RepoMergePRCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			...
			var res = Service.GetComponent<ITGRepository>().MergePullRequest(PR);
			Console.WriteLine(res ?? "Success");
			return res == null ? ExitCode.Normal : ExitCode.ServerError;
		}
	...
	class RepoListPRsCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var data = Service.GetComponent<ITGRepository>().MergedPullRequests(out string error);
			if (data == null)
			{
	...
	class RepoListBackupsCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var data = Service.GetComponent<ITGRepository>().ListBackups(out string error);
			if (data == null)
			{
				Console.WriteLine(error);
				return ExitCode.ServerError;
	...
	class RepoCheckoutCommand : Command
	{
		...
		public override ExitCode Run(IList<string> parameters)
		{
			var res = Service.GetComponent<ITGRepository>().Checkout(parameters[0]);
			Console.WriteLine(res ?? "Success");
			return res == null ? ExitCode.Normal : ExitCode.ServerError;
		}
	}

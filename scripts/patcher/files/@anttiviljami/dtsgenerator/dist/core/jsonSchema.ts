...
export function searchAllSubSchema(
    ...
    function searchOpenApiSubSchema(openApi: OpenApiSchema): void {
        ...
        const setSubIdToParameterObject = (
            obj: { [name: string]: Parameter } | undefined,
            keys: string[]
        ) => setSubIdToAnyObject(setSubIdToParameter, obj, keys);
        function setSubIdToParameter(param: Parameter, keys: string[]): void {
            if ("schema" in param) {
                setSubId(param.schema, keys.concat(param.name));
            }
        }
        const setSubIdToParameterObjectNoName = (
            obj: { [name: string]: Parameter } | undefined,
            keys: string[]
        ) => setSubIdToAnyObject(setSubIdToParameterNoName, obj, keys);
        function setSubIdToParameterNoName(param: Parameter, keys: string[]): void {
            if ("schema" in param) {
        ...
        function setSubIdToParameters(array: Parameter[] | undefined, keys: string[]): void {
            ...
            array.forEach(item => {
                if ("schema" in item) {
                    ...
                    // @ts-ignore
                    const result = (/\/([^\/]*)$/.exec(<string>item.$ref) as RegExpExecArray)[1];
                    // item.$ref = item.$ref + "/schema";
        ...
        function addParameterSchema(
            params: Map<string, ParameterObject[]>,
            refs: Map<string, { $ref?: string }[]>,
            keys: string[]
        ): void {
            for (const [key, param] of params) {
                const [paths, obj] = buildParameterSchema(key, param, keys);
                setSubId(obj, paths);
            }

            for (const [key, ref] of refs) {
                const [paths, obj] = buildParameterSchemaRefs(key, ref, keys);
                setSubId(obj, paths);
            }
        }
        ...
        function buildParameterSchemaRefs(
            inType: string,
            refs: { $ref?: string }[],
            keys: string[]
        ): [string[], JsonSchemaObject] {
            ...
            refs.forEach(item => {
                if (item.$ref != null) {
                    const result = (/\/([^\/]*)$/.exec(<string>item.$ref) as RegExpExecArray)[1];
                    //const ref = "#" + item.$ref.slice(1).split("/").map(convertKeyToTypeName).join("/");
            ...
            return [
                paths,
                {
                    ...
                    properties,
                    required: refs.map(
                        item => (/\/([^\/]*)$/.exec(<string>item.$ref) as RegExpExecArray)[1]
                    )
                }
            ];
        }
        ...
        const setSubIdToResponsesV2 = (
            responses: SwaggerIo.V2.SchemaJson.Definitions.Responses | undefined,
            keys: string[]
        ) => setSubIdToAnyObject(setSubIdToResponseV2, responses, keys);
        function setSubIdToResponseV2(
            response: SwaggerIo.V2.SchemaJson.Definitions.ResponseValue | undefined,
            keys: string[]
        ): void {
            if (response == null) {
                return;
        ...
        function setSubIdToOperationV2(
            ops: SwaggerIo.V2.SchemaJson.Definitions.Operation | undefined,
            keys: string[]
        ): void {
            if (ops == null) {
                return;
        ...
        const setSubIdToPathsV2 = (
            paths: SwaggerIo.V2.SchemaJson.Definitions.Paths,
            keys: string[]
        ...
        function setSubIdToMediaTypes(
            types: OpenApisOrg.V3.SchemaJson.Definitions.MediaTypes | undefined,
            keys: string[]
        ): void {
            if (types == null) {
                return;
        ...
        const setSubIdToRequestBodies = (
            bodys: OpenApisOrg.V3.SchemaJson.Definitions.RequestBodiesOrReferences | undefined,
            keys: string[]
        ) => setSubIdToAnyObject(setSubIdToRequestBody, bodys, keys);
        function setSubIdToRequestBody(
            body: OpenApisOrg.V3.SchemaJson.Definitions.RequestBodyOrReference | undefined,
            keys: string[]
        ): void {
            if (body == null) {
                return;
        ...
        const setSubIdToResponsesV3 = (
            responses: OpenApisOrg.V3.SchemaJson.Definitions.ResponsesOrReferences | undefined,
            keys: string[]
        ) => setSubIdToAnyObject(setSubIdToResponseV3, responses, keys);
        function setSubIdToResponseV3(
            response: OpenApisOrg.V3.SchemaJson.Definitions.ResponseOrReference | undefined,
            keys: string[]
        ): void {
            if (response == null) {
                return;
        ...
        function setSubIdToOperationV3(
            ops: OpenApisOrg.V3.SchemaJson.Definitions.Operation | undefined,
            keys: string[]
        ): void {
            if (ops == null) {
                return;
        ...
        const setSubIdToPathsV3 = (
            paths: OpenApisOrg.V3.SchemaJson.Definitions.Paths,
            keys: string[]
        ) => setSubIdToAnyObject(setSubIdToPathItemV3, paths, keys);
        function setSubIdToPathItemV3(
            pathItem: OpenApisOrg.V3.SchemaJson.Definitions.PathItem,
            keys: string[]
        ): void {
            setSubIdToParameters(pathItem.parameters, keys.concat("parameters"));
        ...
        function setSubId(s: JsonSchema | undefined, paths: string[]): void {
            ...
            if (typeof s.$ref === "string") {
                const thing = "#" + s.$ref.slice(1).split("/").map(convertKeyToTypeName).join("/");
                const schemaId = new SchemaId(thing);
                s.$ref = schemaId.getAbsoluteId();
                onFoundReference(schemaId);
            }

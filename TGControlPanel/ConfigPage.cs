...
namespace TGControlPanel
{
	...
	partial class Main
	{
		...
		void LoadConfig()
		{
			var RepoReady = Service.GetComponent<ITGRepository>().Exists();
		...
		private void RemoveRankButton_Click(object sender, EventArgs e)
		{
			...
			var result = Service.GetComponent<ITGConfig>().RemoveAdminRank(rank);
			if (result != null)
		...
		{
			var result = Service.GetComponent<ITGConfig>().SetAdminRank(AddRankTextBox.Text, new Dictionary<string, bool>());
			if (result != null)
				MessageBox.Show("Error: " + result);
			else
				AddRankTextBox.Text = "";
		...
		private void ApplyAdminRankButton_Click(object sender, EventArgs e)
		{
			...
			admin = admin.Split(' ')[0];
			var result = Service.GetComponent<ITGConfig>().Addmin(admin, rank);
			if (result != null)
				MessageBox.Show("Error: " + result);
			LoadAdminsList();
		}
		...
		private void DeadminButton_Click(object sender, EventArgs e)
		{
			...
			var result = Service.GetComponent<ITGConfig>().Deadmin(selectedAdmin.Split(' ')[0]);
			if (result != null)
				MessageBox.Show("Error: " + result);
			LoadAdminsList();
		}
		...
		private void AddminButton_Click(object sender, EventArgs e)
		{
			...
			var result = Service.GetComponent<ITGConfig>().Addmin(AddminTextBox.Text, rank);
			if (result != null)
				MessageBox.Show("Error: " + result);
			else
				AddminTextBox.Text = "";
		...
		private void AdjustCurrentRankPermissions(object sender, ItemCheckEventArgs e)
		{
			...
			var result = Service.GetComponent<ITGConfig>().SetAdminRank((string)AdminRanksListBox.SelectedItem, perms);
			if (result != null)
				MessageBox.Show("Error: " + result);
		...
		void UpdatePermissionsDisplay() { 
			var rank = (string)AdminRanksListBox.SelectedItem;
			var ranks = Service.GetComponent<ITGConfig>().AdminRanks(out string error);
		...
		void LoadGenericConfig(TGConfigType type)
		{
			...
			var Entries = Service.GetComponent<ITGConfig>().Retrieve(type, out string error);
			if (Entries != null)
		...
		void LoadAdminsList()
		{
			var admins = Service.GetComponent<ITGConfig>().Admins(out string error);
		...
		{
			var perms = Service.GetComponent<ITGConfig>().ListPermissions(out string error);
		...
		void ConfigApply_Click(object sender, EventArgs e)
		{
			switch ((ConfigIndex)ConfigPanels.SelectedIndex)
			{
				...
				case ConfigIndex.Jobs:
					...
					break;
				case ConfigIndex.Maps:
					Config = Service.GetComponent<ITGConfig>();
		...
		void LoadMapsConfig()
		{
			...
			var Maps = Service.GetComponent<ITGConfig>().MapSettings(out string error);
			if (Maps == null)
			{
		...
		void LoadJobsConfig()
		{
			...
			var Jobs = Service.GetComponent<ITGConfig>().Jobs(out string error);
		...
		void ConfigUpload_Click(object sender, EventArgs eva)
		{
			...
			if (error == null)
				error = Service.GetComponent<ITGConfig>().WriteRaw(originalFileName, fileContents);
			if (error != null)
		...
		void DownloadConfig(string remotePath, bool repo)
		{
			if (remotePath == null)
				return;
			var text = Service.GetComponent<ITGConfig>().ReadRaw(remotePath, repo, out string error);
			if (text != null)
			{

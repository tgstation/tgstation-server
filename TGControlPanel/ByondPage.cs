...
namespace TGControlPanel
{
	partial class Main
	{
		...
		void InitBYONDPage()
		{
			...
			var latestVer = Service.GetComponent<ITGByond>(Program.Instance).GetVersion(TGByondVersion.Latest);
		...
		{
			UpdateBYONDButtons();
			if (!Service.GetComponent<ITGByond>(Program.Instance).UpdateToVersion((int)MajorVersionNumeric.Value, (int)MinorVersionNumeric.Value))
		...
		private void BYONDTimer_Tick(object sender, EventArgs e)
		{
			...
			{
				UpdateBYONDButtons();
				var error = Service.GetComponent<ITGByond>(Program.Instance).GetError();

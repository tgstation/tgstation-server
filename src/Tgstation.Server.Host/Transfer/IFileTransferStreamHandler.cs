...
namespace Tgstation.Server.Host.Transfer
{
	...
	public interface IFileTransferStreamHandler
	{
		...
		/// </summary>
		/// <param name="ticket">The <see cref="FileTicketResponse.FileTicket"/>.</param>
		...
		Task<ErrorMessageResponse?> SetUploadStream(string ticket, Stream stream, CancellationToken cancellationToken);
		...
		Task<Tuple<FileStream?, ErrorMessageResponse?>> RetrieveDownloadStream(string ticket, CancellationToken cancellationToken);
	}
}

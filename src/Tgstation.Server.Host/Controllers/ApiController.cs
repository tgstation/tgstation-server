...
namespace Tgstation.Server.Host.Controllers
{
	...
	public abstract class ApiController : Controller
	{
		...
		public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
		{
			...
			Logger.LogTrace("Request made by User ID {0} to Instance {6}. Api version: {1}. User-Agent: {2}. Type: {3}. Route {4}{5}", AuthenticationContext?.User.Id.ToString(CultureInfo.InvariantCulture), ApiHeaders.ApiVersion, ApiHeaders.UserAgent, Request.Method, Request.Path, Request.QueryString, ApiHeaders.InstanceId);

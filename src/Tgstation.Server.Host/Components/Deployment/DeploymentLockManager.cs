...
#pragma warning restore CA2213 // Disposable fields should be disposed
		...
		/// <inheritdoc />
		public ValueTask DisposeAsync() => Interlocked.Exchange(ref firstLock, null)?.DisposeAsync() ?? ValueTask.CompletedTask;
			...
			ValueTask LockCleanupAction()
			{
				...
				lock (locks)
				{
					...
					if (locks.Remove(newLock!))
						...
						logger.LogDebug("A .dmb lock was disposed more than once: {descriptor}", descriptor);

					if (locks.Count == 0)
						return dmbProvider.DisposeAsync();
				}

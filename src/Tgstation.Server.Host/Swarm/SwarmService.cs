...
namespace Tgstation.Server.Host.Swarm
{
	...
	sealed class SwarmService : ISwarmService, ISwarmOperations, IRestartHandler, IAsyncDisposable
	{
		...
		[MemberNotNullWhen(
			true,
			nameof(swarmServers),
			nameof(registrationIds),
			nameof(updateSynchronizationLock),
			nameof(forceHealthCheckTcs),
			nameof(localSwarmServer),
		...
		/// <summary>
		/// The <see cref="SwarmServerResponse"/> for the local server.
		/// </summary>
		readonly SwarmServerResponse? localSwarmServer;

		/// <summary>
		/// The <see cref="Uri"/> of the swarm controller.
		/// </summary>
		readonly Uri? controllerAddress;
		...
		/// <summary>
		/// The <see cref="SwarmConfiguration.PrivateKey"/>.
		/// </summary>
		...
		/// <param name="swarmConfigurationOptions">The <see cref="IOptions{TOptions}"/> containing the <see cref="SwarmConfiguration"/> for the <see cref="SwarmService"/>.</param>
		/// <param name="generalConfigurationOptions">The <see cref="IOptions{TOptions}"/> containing the <see cref="GeneralConfiguration"/> for the <see cref="SwarmService"/>.</param>
			...
			ILogger<SwarmService> logger)
		{
			...
			SwarmMode = !String.IsNullOrWhiteSpace(swarmConfiguration.PrivateKey);
			if (SwarmMode)
			{
				privateKey = swarmConfiguration.PrivateKey;
			...
			if (SwarmMode)
			{
				...
				if (swarmController)
				{
					controllerAddress = new Uri($"http://127.0.0.1:{generalConfiguration.ApiPort}");
					registrationIds = new Dictionary<string, Guid>();
				}
				else
					controllerAddress = swarmConfiguration.ControllerAddress;

				localSwarmServer = new SwarmServerResponse
				{
		...
		public async Task AbortUpdate(CancellationToken cancellationToken)
		{
			...
			async Task SendRemoteAbort(SwarmServerResponse swarmServer)
			{
				...
				catch (Exception ex)
				{
					logger.LogWarning(
						ex,
						"Unable to send remote abort to {abortTarget}!",
						swarmController
		...
		public async Task<bool> CommitUpdate(CancellationToken cancellationToken)
		{
			...
			async Task SendRemoteCommitUpdate(SwarmServerResponse swarmServer)
			{
				...
				{
					logger.LogCritical(ex, "Failed to send update commit request to node {identifier}!", swarmServer.Identifier);
				}
			}
		...
		{
			logger.LogTrace("Received remote update request from {source}", !swarmController ? "controller" : "node");
		...
		public async Task<SwarmRegistrationResult> Initialize(CancellationToken cancellationToken)
		{
			if (SwarmMode)
				logger.LogInformation(
					"Swarm mode enabled: {type} {identifier}",
					swarmController
						? "Controller"
						: "Node",
					localSwarmServer.Identifier);
			else
		...
		public async Task Shutdown(CancellationToken cancellationToken)
		{
			...
			if (SwarmMode && !swarmController)
			{
				...
				if (!restarting)
				{
					...
					await SendUnregistrationRequest(new SwarmServerResponse
					{
						Address = controllerAddress,
					});
				}
		...
		public bool RegisterNode(Api.Models.Internal.SwarmServer node, Guid registrationId)
		{
			...
			lock (updateSynchronizationLock)
			{
				if (targetUpdateVersion != null)
				{
					logger.LogInformation("Not registering node {identifier} as a distributed update is in progress.", node.Identifier);
					return false;
				}
				...
				lock (swarmServers)
				{
					if (registrationIds.Any(x => x.Value == registrationId))
					{
						...
						{
							logger.LogWarning("Node {identifier} has already registered!", node.Identifier);
							return true;
						}

						logger.LogWarning(
							"Registration ID collision! Node {identifier} tried to register with {otherIdentifier}'s registration ID: {registrationId}",
							node.Identifier,
					...
					{
						logger.LogInformation("Node {identifier} is re-registering without first unregistering. Indicative of restart.", node.Identifier);
			...
			logger.LogInformation("Registered node {identifer} ({address}) with ID {registrationId}", node.Identifier, node.Address, registrationId);
			MarkServersDirty();
			return true;
		}
		...
		public async Task<bool> RemoteCommitRecieved(Guid registrationId, CancellationToken cancellationToken)
		{
			...
			if (nodeList == null)
			{
				logger.LogDebug("Ignoring ready-commit from node {identifier} as the update appears to have been aborted.", nodeIdentifier);
				return false;
			}

			logger.LogDebug("Node {identifier} is ready to commit.", nodeIdentifier);
			lock (nodeList)
			{
		...
		public async Task UnregisterNode(Guid registrationId, CancellationToken cancellationToken)
		{
			...
			logger.LogInformation("Unregistering node {identifier}...", nodeIdentifier);
			await AbortUpdate(cancellationToken).ConfigureAwait(false);
			lock (swarmServers)
			{
		...
		async Task<bool> PrepareUpdateImpl(Version version, bool initiator, CancellationToken cancellationToken)
		{
			...
			async Task<bool> RemotePrepareUpdate(SwarmServerResponse? swarmServer)
			{
				using var request = PrepareSwarmRequest(
					...
					HttpMethod.Put,
					SwarmConstants.UpdateRoute,
					new SwarmUpdateRequest(version));
			...
			try
			{
				lock (updateSynchronizationLock)
				{
					...
					if (targetUpdateVersion != null)
					{
						logger.LogDebug("Aborting update preparation, version {version} already prepared!", targetUpdateVersion);
						shouldAbort = true;
						return false;
					}
			...
			try
			{
				...
				{
					logger.LogDebug("Distributed prepare for update to version {version} complete.", version);
					return true;
				}
			}
			catch (Exception ex)
		...
		async Task HealthCheckNodes(CancellationToken cancellationToken)
		{
			...
			async Task HealthRequestForServer(SwarmServerResponse swarmServer)
			{
				...
				catch (Exception ex) when (ex is not OperationCanceledException)
				{
					logger.LogWarning(
						ex,
						"Error during swarm server health check on node '{identifier}'! Unregistering...",
		...
		{
			if (!SwarmMode)
				throw new InvalidOperationException("Cannot RegisterWithController while not in swarm mode!");

			logger.LogInformation("Attempting to register with swarm controller at {address}...", controllerAddress);
			...
			using var registrationRequest = PrepareSwarmRequest(
				...
				new SwarmRegistrationRequest
				{
					...
					Identifier = localSwarmServer.Identifier,
					Address = localSwarmServer.Address,
				},
		...
		async Task SendUpdatedServerListToNodes(CancellationToken cancellationToken)
		{
			...
			List<SwarmServerResponse> currentSwarmServers;
			lock (swarmServers)
				currentSwarmServers = new (swarmServers);
			...
			async Task UpdateRequestForServer(SwarmServerResponse swarmServer)
			{
				using var request = PrepareSwarmRequest(
					...
					{
						SwarmServers = new List<SwarmServerResponse?>(currentSwarmServers),
					});
				...
				{
					logger.LogWarning(ex, "Error during swarm server list update for node '{identifier}'! Unregistering...", swarmServer.Identifier);
			...
			Guid? registrationIdOverride = null)
		{
			...
			logger.LogTrace(
				"{httpMethod} {route} to swarm server {address}",
				httpMethod,
				subroute,
				swarmServer.Address.ToString());

			var request = new HttpRequestMessage(
				httpMethod,
				swarmServer.Address + subroute[1..]);

			request.Headers.Add(SwarmConstants.ApiKeyHeader, privateKey);
			...
			try
			{
				...
				while (!cancellationToken.IsCancellationRequested)
				{
					...
					if (nextForceHealthCheckTask.IsCompleted && swarmController)
					{
						...
						logger.LogTrace("Next health check triggering in {seconds}s...", SecondsToDelayForcedHealthChecks);
		...
		string? NodeIdentifierFromRegistration(Guid registrationId)
		{
			...
			lock (swarmServers)
			{
				...
				if (!exists)
				{
					logger.LogDebug("A node that was to be looked up ({registrationId}) disappeared from our records!", registrationId);
					return null;
				}

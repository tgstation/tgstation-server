...
namespace Tgstation.Server.Host.Components.Byond.Tests
{
	...
	public sealed class TestPosixByondInstaller
	{
		...
		public async Task TestDownload()
		{
			...
			mockFileDownloader.Setup(x => x.DownloadFile(It.Is<Uri>(uri => uri == new Uri("https://secure.byond.com/download/build/511/511.1385_byond_linux.zip")), null, default)).Returns(Task.FromResult<Stream>(new MemoryStream(ourArray))).Verifiable();

			var result = await installer.DownloadVersion(new Version(511, 1385), default);

			Assert.AreSame(ourArray, ((MemoryStream)result).ToArray());
